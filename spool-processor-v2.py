...
def extract_via_ollama(transcript):
    ...
    system_prompt = (
        "You are a structured data extractor for an IT infrastructure knowledge base. "
        "You extract facts from conversation logs into a strict format. "
        "You write in past tense. You never fabricate details. "
        "Output ONLY the sections requested, no commentary.\n\n"
        "ENTITY NAMING RULES (critical):\n"
        "1. Use hostnames for machines: vader, voldemort, gargamel, skynet, pihole, "
        "docker-lxc, proxmox, unraid. NEVER use hardware names as entity names.\n"
        "2. Use short lowercase names for services: ollama, deal-finder, chroma, "
        "n8n, gitea, spool-processor. Not 'Deal Finder MCP service'.\n"
        "3. Every fact MUST use an entity name that appears in your Entities list. "
    ...
    prompt = (
        ...
        + transcript
        + "\n</CONVERSATION_LOG>\n\n"
        "Output EXACTLY this format. No bold, no extra headers, no markdown formatting within sections.\n\n"
        "## Summary\n"
        "One paragraph.\n\n"
        "## Entities\n"
        "List every machine, service, tool, or project mentioned. One per line:\n"
        "- entity_name | type | one-line description\n"
        "Types: machine, service, tool, project, config, device\n"
        "Example:\n"
        "- vader | machine | Mac Studio M3 Ultra, 256GB RAM\n"
        "- docker-lxc | machine | Proxmox LXC container at 192.168.0.144\n"
        ...
        "(Write 'None' if no problems were encountered.)\n"
    )
